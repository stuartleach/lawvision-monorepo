...
<script lang="ts">
	...
	import { selectionStore } from '$lib/stores/data';
	import type { Writable } from 'svelte/store';

	export let label = 'Select Option';
	export let options: any[];
	export let type: string = '';
	export let setter: () => void = () => {
	};

	let eachOptions: { name: string, value: any }[] = [];
	let selectedOption: any = options[0] || '';

	const selectOption = (option) => {
		selectedOption = option;
		selectionStore.update((store) => {
			store[type] = option.value;
			return store;
		});
	};
	...
	$: {
		...
		options?.forEach((option) => {
			switch (type) {
				...
				case 'metric':
				case 'direction':
					eachOptions.push({ name: option.name || option, value: option });
					break;
				default:
					break;
			}
	...
	<div class="relative mt-2">
		...
		{#if expanded}
			<ul
				...
				role="listbox"
				aria-labelledby="listbox-label">
				{#each eachOptions as option}
					<li
						class="relative cursor-default select-none py-2 pl-3 pr-9 text-gray-900"
						id="listbox-option-{option.value}"
						role="option"
						on:click={() => {selectOption(option); expanded = !expanded}}>
