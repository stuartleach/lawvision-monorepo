...
>
	...
	<div class="text-left sm:pl-5">
		<h4 class="mb-1 text-base hidden sm:block sm:text-xl font-bold tracking-tight text-gray-500">{selectedJudgeInfo ? 'The Honorable' : ''}</h4>
		<h2 class="mb-1 sm:text-6xl block sm:block text-xl font-semibold tracking-tight text-gray-50">
			{selectedJudgeInfo?.name || ''}
		</h2>
	</div>
	<h2
		...
		{selectedJudgeInfo ? ' Trial Judge in ' : ''} <span
		class="bg-gradient-to-tr from-red-500 to-yellow-300 bg-clip-text font-bold text-transparent"
	>
			{selectedJudgeInfo ? selectedJudgeInfo?.primaryCounty +' County' : ''}
		</span
	>
	</h2>
</div>
