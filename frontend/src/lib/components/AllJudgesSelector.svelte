<script lang="ts">
	...
	import type { CountyExpandedProperties, CountyFeature, County, Judge } from '$lib/types/types';
	import {
		allCountiesStore,
		selectedCountyStore,
		selectedJudgeStore,
				...
				{#each allJudges.slice(0, 100) as judge, index}
					<button on:click={() => selectedJudgeStore.set(judge)}
						...
						<div class="font-mono  w-full">
							{#if metric === 'bail'}
								<Money
									value={hoveredStat === 'amount' ? Number(judge) * judge.cases_bail_set : judge.average_bail_set} />
