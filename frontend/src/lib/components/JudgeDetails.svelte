<script lang="ts">
	...
	import type { Judge, Judge } from '$lib/types/types';
...
<div class="judge-detail">
	...
	<h4>{selectedJudgeInfo?.name ? "The Honorable Judge" : ""}</h4>
	...
	<div>
		<ul class="space-y-2">
			<li class="stat">
				...
				<p
					class="font-bold text-right text-zinc-400 font-mono">{selectedJudgeInfo ? formatNumber(selectedJudgeInfo.stats.raw.totalCases) : '0'}</p>
			</li>
			<li class="stat" on:mouseenter={() => handleMouseEnter('amount')} on:mouseleave={handleMouseLeave}>
				...
				<div class="right text-right">
					<div class="money flex-row text-right">
						...
						<span
							class="font-bold text-white font-mono">{selectedJudgeInfo ? formatMoney(hoveredStat === 'amount' ? selectedJudgeInfo.stats.raw.bail * selectedJudgeInfo.average_bail_set : selectedJudgeInfo.average_bail_set).split('.')[0] : '0'}</span>&nbsp;<span
