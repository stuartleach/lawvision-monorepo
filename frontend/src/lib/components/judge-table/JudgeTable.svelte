				...
				<tr class:bg-zinc-950={i % 2 === 0}
					...
					<td class="py-4 pl-4 pr-8 sm:pl-6 lg:pl-8">
						<div class="flex items-center gap-x-4">
							<div
								class="truncate hover:text-gray-50 text-lg leading-6">{judge.name}</div>
						</div>
					</td>
					...
					<td
						class="hidden py-4 pl-0 pr-8 leading-6 md:table-cell text-lg lg:pr-20 text-right bailSet-color font-mono {$sortTarget === SortTarget.bailSet ? 'font-bold' : ''}">
						<Percent value={judge.stats.pct.bailSet} />
					</td>
					<td
						class="hidden py-4 pl-0 pr-8 text-lg leading-6 md:table-cell lg:pr-20 text-right remand-color font-mono {$sortTarget === SortTarget.remandPct ? 'font-bold' : ''}">
