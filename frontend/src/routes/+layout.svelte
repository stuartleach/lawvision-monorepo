<script lang="ts">
	...
	import EntityProfile from '$lib/components/judge-profile/EntityProfile.svelte';
...
<div class="flex flex-row justify-start">
	<div class="h-screen lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
		...
		<div
			class="flex grow flex-col gap-y-5 py-4 overflow-y-auto text-zinc-300 border-r border-zinc-700/50 bg-zinc-950">
			<div class="flex flex-col justify-start">
				<Button class="flex justify-start" on:click={()=>clearSelection()}>
					<h1
						class="text-5xl  tracking-[-.04em] bg-gradient-to-tr from-red-500 to-yellow-300 bg-clip-text font-bold text-transparent">
						LawVision
					</h1>
				</Button>
			</div>
	...
	<div class=" z-40 flex items-center gap-x-6 bg-zinc-900 px-4 py-4 shadow-sm sm:px-6 lg:hidden">
		...
		<a href="#">
			<h1
				class="text-left text-4xl tracking-[-.04em] bg-gradient-to-tr from-red-500 to-yellow-300 bg-clip-text font-bold text-transparent">
				LawVision
			</h1>
	...
	<!--{#if $currentPageStore === 'judges' || $currentPageStore === 'counties'}-->
	<div
		class="overflow-y-scroll h-screen border-r border-zinc-800 bg-zinc-900">
		<ListOfItems />
	</div>
	<!--{/if}-->
	<main class="">
		<div class="">
			<div class="px-4 py-10 sm:px-6 lg:px-8 lg:py-6 bg-zinc-950 h-screen">
